...
const selectEnvelope = `SELECT id, user_id, category_id, account_id, dest_account_id, mode, default_amount, frequency,
...
func (envelopeRepo) Create(ctx context.Context, q DBTX, e *domain.Envelope) (int64, error) {
	res, err := q.ExecContext(ctx,
		`INSERT INTO envelope (user_id, category_id, account_id, dest_account_id, mode, default_amount, frequency,
			due_months, expected_day, status, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		e.UserID, e.CategoryID, e.AccountID, nullInt64(e.DestAccountID), string(e.Mode), nullInt64(e.DefaultAmount), nullFrequency(e.Frequency),
...
func (envelopeRepo) Update(ctx context.Context, q DBTX, e *domain.Envelope) error {
	_, err := q.ExecContext(ctx,
		`UPDATE envelope SET category_id = ?, account_id = ?, dest_account_id = ?, mode = ?, default_amount = ?, frequency = ?,
			due_months = ?, expected_day = ?, status = ?, updated_at = ?
		 WHERE user_id = ? AND id = ?`,
		e.CategoryID, e.AccountID, nullInt64(e.DestAccountID), string(e.Mode), nullInt64(e.DefaultAmount), nullFrequency(e.Frequency),
...
func scanEnvelope(row rowScanner) (*domain.Envelope, error) {
	...
	err := row.Scan(&e.ID, &e.UserID, &e.CategoryID, &e.AccountID, &destAccountID, &e.Mode, &defaultAmount, &frequency,
	...
	e.DestAccountID = ptrInt64(destAccountID)
